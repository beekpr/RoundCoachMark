...
class CoachRingView: UIView, CAAnimationDelegate {
    ...
    private func animateRing(_ open: Bool) {
        ...
        let timing: CAMediaTimingFunction = CAMediaTimingFunction(name: kCAMediaTimingFunctionEaseInEaseOut)
        let endRadius = open ? ring.radius : ring.controlRadius
        let begRadius = open ? ring.controlRadius : ring.radius
        ...
        opening.fromValue = begRadius
        opening.toValue = endRadius
        opening.delegate = self
        layer.add(opening, forKey: "ring")

        CATransaction.begin()
        CATransaction.setDisableActions(true)
        (layer as? CMRingLayer)?.ringRadius = endRadius
    ...
    private func animateEcho(_ open: Bool) {
        guard let ring = ringGeometry else { return }
        let echoTiming: CAMediaTimingFunction = CAMediaTimingFunction(name: kCAMediaTimingFunctionDefault)
        let opacityTiming: CAMediaTimingFunction = CAMediaTimingFunction(name: kCAMediaTimingFunctionLinear)
        ...
        pulse.timingFunction = echoTiming
        pulse.fromValue = ring.controlRadius
        ...
        opacity.timingFunction = opacityTiming
    ...
    override func draw(_ layer: CALayer, in ctx: CGContext) {
        guard let ring = ringGeometry,
            let ringRadius = (layer as? CMRingLayer)?.ringRadius,
            let controlRadius = (layer as? CMRingLayer)?.aperture,
            let echoRadius = (layer as? CMRingLayer)?.echo,
            let echoOpacity = (layer as? CMRingLayer)?.echoOpacity else { return }
