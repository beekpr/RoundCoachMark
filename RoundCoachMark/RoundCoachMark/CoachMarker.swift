...
public class CoachMarker {
    ...
    private func setup() {
        guard let container = marksContainer else { return }
        container.addSubview(marksCanvas)
    ...
    @discardableResult public static func registerMark(position: CGPoint, aperture: CGFloat, title: String, info: String, control: Any? = nil, autorelease: Bool = true, markTag: String? = nil) -> CoachMarkHandler {
        let handler = CoachMarkHandler()
        handler.token = NotificationCenter.default.addObserver(forName: Events.CoachMarkerMarksRequest, object: nil, queue: OperationQueue.main) { note in
    ...
    @discardableResult public static func registerMark(title: String, info: String, centerShift: CGPoint = CGPoint.zero, aperture: CGFloat = 0, control: UIView, autorelease: Bool = true, markTag: String? = nil) -> CoachMarkHandler {
        let handler = CoachMarkHandler()
        handler.token = NotificationCenter.default.addObserver(forName: Events.CoachMarkerMarksRequest, object: nil, queue: OperationQueue.main) { note in
            guard let marker = note.object as? CoachMarker else { return }
            guard marker.tagsAccordance(markTag) else { return }
    ...
    @discardableResult public static func registerMark(position: CGPoint, aperture: CGFloat, info: Any, control: Any? = nil, autorelease: Bool = true, markTag: String? = nil) -> CoachMarkHandler {
        let handler = CoachMarkHandler()
        handler.token = NotificationCenter.default.addObserver(forName: Events.CoachMarkerMarksRequest, object: nil, queue: OperationQueue.main) { note in
            guard let marker = note.object as? CoachMarker else { return }
            guard marker.tagsAccordance(markTag) else { return }
    ...
    @discardableResult public static func registerMark(position: CGPoint, aperture: CGFloat, info: Any?, infoView: CoachMarkInfoView, control: Any? = nil, autorelease: Bool = true, markTag: String? = nil) -> CoachMarkHandler {
        let handler = CoachMarkHandler()
        handler.token = NotificationCenter.default.addObserver(forName: Events.CoachMarkerMarksRequest, object: nil, queue: OperationQueue.main) { note in
            guard let marker = note.object as? CoachMarker else { return }
            guard marker.tagsAccordance(markTag) else { return }
    ...
    public static func unregisterMark(_ handler: CoachMarkHandler) {
    ...
    public func addMark(title: String, info: String, centerShift: CGPoint = CGPoint.zero, aperture: CGFloat = 0, control: UIView) {
        guard let controlSuperview = control.superview else {
            print("The control's view is not in hierarchy. The mark cannot be aded")
            return
        }
        let position = marksCanvas.convert(control.center, from: controlSuperview).applying(CGAffineTransform.identity.translatedBy(x: centerShift.x, y: centerShift.y))
        let theAperture = aperture > 0 ? aperture : max(control.bounds.size.width, control.bounds.size.height) + 6
        let mark = MarkInfo(position: position, aperture: theAperture, control: control, textInfo: (title, info), info: nil, infoView: nil)
        marks.append(mark)
    }
    ...
    private func tagsAccordance(_ markTag: String?) -> Bool {
        if let markterTag = markerTag {
            guard let markTag = markTag else { return false }
            return markterTag == markTag
        }
        if let markTag = markTag {
            guard let markerTag = markerTag else { return false }
            return markTag == markerTag
        }
        return false
    }
    ...
    public func autoPlay(delay: Double, interval: Double, destroyWhenFinished: Bool = true, completion: (() -> Void)? = nil) {
        ...
        func fireTimer() {
            showTimer = Timer.scheduledTimer(timeInterval: interval, target: self, selector: #selector(playNext(_:)), userInfo: nil, repeats: true)
            showTimer?.fire()
        }
    ...
    @objc private func playNext(_ sender: Any) {
        ...
        animating = true
        if showCount == nil { showCount = marksCount }
        if showCount! == 0 {
            animating = false
            if sender is Timer { showTimer?.invalidate(); showTimer = nil }
            if destroyAfterPlay {
                playCompletion?()
                destroy { [weak self] in
    ...
    func validateContainer() -> Bool {
        guard marksContainer != nil else { print("\(self) is not added into a view hierarchy. Check if the container view passed to the constructor exists and is in an hierarcy"); return false }
        return true
    }
}
...
extension CoachMarker {
    public static func cycleAllMarksOnce(in _: CoachMarker) -> Bool {
