...
extension String {
    func calculateSize(for view: UITextView) -> CGSize {
        ...
        let probeSize = CGSize(width: view.bounds.width - insets.left - insets.right - padding, height: CGFloat(MAXFLOAT))
        let font = view.font ?? UIFont.systemFont(ofSize: 10)
        let size = (self as NSString).boundingRect(with: probeSize,
    ...
    func calculateSize(width: CGFloat, font: UIFont) -> CGSize {
        let probeSize = CGSize(width: width, height: CGFloat(MAXFLOAT))
        let size = (self + "a").boundingRect(with: probeSize,
